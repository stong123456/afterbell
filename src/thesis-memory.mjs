...
export function createThesis({symbol,horizon,idea,assumptions},now=Date.now()){
 if(!/^[A-Z0-9.]{1,16}$/.test(symbol)||idea.trim().length<10||idea.length>2000||!assumptions.length||assumptions.length>8||assumptions.some(a=>!a.text.trim()))throw Error('INVALID_THESIS');
 return {version:1,id:crypto.randomUUID(),symbol,horizon:horizon.slice(0,100),idea,createdAt:new Date(now).toISOString(),assumptions:assumptions.map((a,i)=>({id:String(i),text:a.text.slice(0,500),status:'unverified'})),history:[],seen:[],closed:false};
}
