...
export function challenge(thesis,event){
 if(typeof thesis!=='string'||thesis.trim().length<10||thesis.length>2000)throw Error('请填写 10–2000 字的交易观点。');
 const rebound=/反弹|超跌|overdone|rebound|buy|买/i.test(thesis);
 return {mode:'rules',createdAt:new Date().toISOString(),thesis,event:event.title,verdict:'证据不足，暂不能确认交易假设',probability:null,checks:[{title:rebound?'下跌本身不能证明超跌':'把方向判断改写成可证伪假设',body:rebound?'需要证明价格变化超过基本面影响；价格跌幅不能单独支持反弹。':'明确标的、时间窗口、触发条件，以及什么证据会推翻你的观点。'},{title:'检查事件传导',body:event.mechanism},{title:'主动寻找反证',body:event.counter},{title:'检查市场可交易性',body:'核实 rToken 身份、报价时间、买卖价差、流动性和赎回约束；缺少这些信息时不能判断价格偏离。'}],missing:['尚无可验证历史相似事件样本','尚无校准后的开盘 Gap 概率模型',event.kind==='scenario'?'情景内容不等同于已发生新闻':'来源正文与交易影响尚未人工核实'],next:event.checks};
}
