...
export async function qwenChallenge(thesis,event,evidence,lang){
 const config=modelConfig();if(!config.configured)throw Error('QWEN_NOT_CONFIGURED');
 const base=new URL(process.env.QWEN_BASE_URL||'https://dashscope.aliyuncs.com/compatible-mode/v1');
 if(base.protocol!=='https:'||!/(^|\.)(aliyuncs\.com|alibabacloud\.com)$/.test(base.hostname))throw Error('QWEN_ENDPOINT_INVALID');
 ...
 const r=await fetch(base.href.replace(/\/$/,'')+'/chat/completions',{method:'POST',signal:AbortSignal.timeout(45000),headers:{'Content-Type':'application/json',Authorization:`Bearer ${process.env.DASHSCOPE_API_KEY}`},body:JSON.stringify({model:config.model,messages:[{role:'system',content:system},{role:'user',content:JSON.stringify({thesis,eventKind:event.kind,evidence})}],response_format:{type:'json_object'},enable_thinking:false,max_tokens:1800,temperature:0.2})});
 if(!r.ok)throw Error(`QWEN_HTTP_${r.status}`);
 ...
 return {...validateAnalysis(value,evidence),mode:'qwen',model:config.model,promptVersion:PROMPT_VERSION,createdAt:new Date().toISOString(),thesis,event:event.title,probability:null,evidence,language:lang};
}
