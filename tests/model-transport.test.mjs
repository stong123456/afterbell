...
test('competition credentials use only the documented Responses gateway',async()=>{
 ...
 try{globalThis.fetch=async(url,options)=>{assert.equal(url,'https://hackathon.bitgetops.com/v1/responses');assert.equal(options.redirect,'manual');const body=JSON.parse(options.body);assert.equal(body.store,false);assert.equal(body.model,'qwen3.8-max');assert.equal(body.input[0].content,'Test');assert.equal(body.response_format,undefined);return new Response(JSON.stringify({status:incomplete?'incomplete':'completed',output:[{type:'message',content:[{type:'output_text',text:'{"ok":true}'}]}]}));};
