...
Every demo model attempt reserves D1 quota: 15 per IP per UTC day, 300 globally per UTC day; failed attempts count. Daily salted hashes replace raw IPs in stored counters, which expire after 2 days. Reservations use conditional atomic upserts, and requests fail closed if DB or trusted Cloudflare IP is unavailable. Maximum model output is 1800 tokens; model timeout is 45 seconds. Two concurrent requests per worker isolate is a secondary control, not a global concurrency guarantee.
