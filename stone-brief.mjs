...
export async function checkWhatChanged(raw,env={},permit=async()=>{}){
 ...
 const response=await modelFetch(config.base+'/chat/completions',{method:'POST',redirect:'error',signal:AbortSignal.timeout(config.provider==='bitget-qwen'?90000:45000),headers:{'Content-Type':'application/json',Authorization:'Bearer '+config.apiKey},body:JSON.stringify({model:config.model,messages:[{role:'system',content:system},{role:'user',content:JSON.stringify({thesis:input.idea,assumptions:input.assumptions,baselineAt:input.baselineAt,reviewSince:input.reviewSince,since:input.reviewSince,evidence})}],response_format:{type:'json_object'},...(config.provider.startsWith('qwen')?{enable_thinking:false}:{}),max_tokens:1800,temperature:0.2})});
