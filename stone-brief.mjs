...
export async function checkWhatChanged(raw,env={},permit=async()=>{}){
 ...
 evidence=validateSelection(selected,input.assumptions,evidence);if(!own)noteAISuccess(config.provider,config.model);base.evidence=evidence;base.coverage.candidateRecords=candidateCount;base.coverage.selectedRecords=evidence.length;base.coverage.retrieval='bilingual-rules-then-assumption-reranking';
 if(!evidence.length)return {...base,mode:'ai',provider:config.provider,model:config.model,results:input.assumptions.map(a=>({assumptionId:a.id,status:'unclear',reason:input.lang==='en'?'The candidate sources do not directly test this assumption.':'候选来源未直接检验这项假设。',evidenceIds:[]}))};
